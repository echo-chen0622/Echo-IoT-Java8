...
jarfile="${BASE}/echoiot-${project.version}-boot.jar"
...
sudo -u "$run_user" -s /bin/sh -c "java -cp ${jarfile} $JAVA_OPTS -Dloader.main=org.echoiot.server.EchoiotInstallApplication \
